...
export function initializeOrientation(labels,beforeToggle){
  let requested=false;const activePointers=new Set(),root=document.getElementById('app');
  const toggle=document.createElement('button');toggle.type='button';toggle.className='orientation-toggle';toggle.setAttribute('aria-pressed','false');root.appendChild(toggle);
  function update(){

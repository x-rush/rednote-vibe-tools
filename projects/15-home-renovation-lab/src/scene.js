...
this.dirty=true;}
  ...
  rebuild(plan){
    ...
    for(const wall of walls(plan)){
      ...
      let cursor=wall.start;for(const o of open){const left=o.center-o.w/2,right=o.center+o.w/2;part(cursor,left,0,h);part(left,right,0,o.sill);part(left,right,o.sill+o.h,h);
        ...
        if(o.type==='door'){const frame=mat('#d4c4ae');for(const [center,y,width,height]of[[left,.5*o.h,.035,o.h],[right,.5*o.h,.035,o.h],[o.center,o.h,o.w+.035,.035]]){const m=box(g,wall.axis==='x'?width:th+.035,height,wall.axis==='x'?th+.035:width,wall.axis==='x'?center:wall.line,y,wall.axis==='x'?wall.line:center,frame);m.userData={kind:'opening',id:o.id,roomId:o.roomId};}}
        cursor=right;
  ...
  select(selected){for(const handle of this.sideHandles||[]){this.root.remove(handle);dispose(handle);}this.sideHandles=[];if(this.guides){this.root.remove(this.guides);dispose(this.guides);this.guides=null;}this.selected=selected;if(this.selector){this.root.remove(this.selector);dispose(this.selector);this.selector=null;}if(this.handle){this.root.remove(this.handle);dispose(this.handle);this.handle=null;}
  ...
  down(e){
    ...
    if(this.walk){this.walkRoute=null;this.controls.enabled=false;this.renderer.domElement.setPointerCapture(e.pointerId);return;}
    if(this.readonly)return;
    if(this.pending){this.controls.enabled=false;return;}
    const hit=this.hit(e),data=hit?.object.userData;if(['item','room','resize','resize-side'].includes(data?.kind)&&this.selected?.id!==data.id)return;if(data?.kind==='item'&&!this.editRooms){if(this.cb.select({kind:'item',id:data.id})===false)return;const o=this.plan.items.find(i=>i.id===data.id);this.drag={kind:'item',id:o.id,original:{...o},candidate:{...o}};this.controls.enabled=false;this.renderer.domElement.setPointerCapture(e.pointerId);}
...
this.drag.candidate={...o,w:snap(Math.max(1.5,o.w+dx)),d:snap(Math.max(1.5,o.d+dz))};if(this.snap)this.drag.candidate=snapRoom(this.plan,this.drag.candidate,o);this.handle?.position.set(o.x+this.drag.candidate.w,.09,o.z+this.drag.candidate.d);}
  ...
  up(e){
    this.activePointers?.delete(e.pointerId);if(this.activePointers?.size){this.start=null;return;}if(!this.start)return;const moved=Math.hypot(e.clientX-this.start.x,e.clientY-this.start.y)>5;

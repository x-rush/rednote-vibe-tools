import {clone,uid,round,bounds,roomAt,validRoom,placement,itemFrom,findSpot,makePlan,inventory,History,validatePlan,csvCell,validOpening,walls,wallOpenings,outsideItems,removeRoom} from './core.js';
import {RoomScene} from './scene.js';
import {adjacentRoom,snapRoom} from './interaction.js';
import {initializeOrientation} from './orientation.js';
import {blockedDoorItems} from './circulation.js';
...
// Only replace exact, unmodified legacy seeds; custom projects remain untouched.
function templateFingerprint(value){
 const copy=clone(value),roomIds=new Map(copy.rooms.map((r,i)=>[r.id,i]));
 delete copy.id;delete copy.name;delete copy.updatedAt;
 for(const r of copy.rooms)delete r.id;
 for(const key of ['items','openings']){
  for(const item of copy[key]){delete item.id;item.roomId=roomIds.get(item.roomId);}
 }
 const stable=value=>Array.isArray(value)?value.map(stable):value&&typeof value==='object'?Object.fromEntries(Object.keys(value).sort().map(k=>[k,stable(value[k])])):value;
 for(const key of ['items','openings'])copy[key]=copy[key].map(stable).sort((a,b)=>JSON.stringify(a).localeCompare(JSON.stringify(b)));
...
if(db.plans.length){
 ...
 db.plans=db.plans.map(saved=>{
  ...
  const previousBaseline=db.baselines[saved.id];
  if(previousBaseline&&templateFingerprint(previousBaseline)!==seed.fingerprint)return saved;
  const refreshed=makePlan(seed.template,catalog);refreshed.id=saved.id;refreshed.name=saved.name;
  db.baselines[saved.id]=clone(refreshed);return refreshed;
 });
}
...
function renderChrome(){
  ...
  let warning=$('#outside-notice');if(!warning){warning=document.createElement('div');warning.id='outside-notice';context.before(warning);}
...
function updateRoom(candidate){
  ...
  const next=clone(plan),old=next.rooms.find(r=>r.id===candidate.id),dx=candidate.x-old.x,dz=candidate.z-old.z,moving=old.w===candidate.w&&old.d===candidate.d;
  Object.assign(old,candidate);if(moving)for(const o of next.items.filter(o=>o.roomId===old.id)){o.x=round(o.x+dx);o.z=round(o.z+dz);}
  if(!next.openings.every(o=>validOpening(next,o))){toast('roomBlocksOpening');scene?.rebuild(plan);return false;}
  commit(p=>Object.assign(p,next),'roomSaved');return true;
}
function enterRoom(r,corner=false){scene?.setPending(null);if(!scene?.walk)selectionBeforeWalk=selected?{...selected}:null;if(scene?.enter(r,corner)){selected={kind:'room',id:r.id};drawer=false;roomPropertiesOpen=false;renderChrome();if(!walkHintShown){toast('walkHint');walkHintShown=true;}}}
...
function saveRoomDraft(forceId=null){
  ...
  for(const f of forms){if(!f.checkValidity())return fail(f,'invalidNumber');const fd=new FormData(f),num=k=>Number(fd.get(k));
    ...
    if(f.id==='global-form'){Object.assign(next,{wallHeight:num('wallHeight'),wallThickness:num('wallThickness'),wallHeightMeasured:fd.has('wallHeightMeasured'),buildingArea:fd.get('buildingArea')===''?null:num('buildingArea')});Object.assign(next.rooms.find(r=>r.id===rid),{textureScale:num('textureScale'),textureAngle:num('textureAngle')});}
...
function placeDoorAt(e){
  const r=plan.rooms.find(r=>r.id===doorRoomId),hit=scene?.hit?.(e),p=['wall','opening'].includes(hit?.object?.userData?.kind)&&hit.point?hit.point:scene?.point(e);if(!r||!p)return;
...
app.addEventListener('click',async e=>{
  ...
  switch(action){
    ...
    case 'delete-room':if(plan.rooms.length===1)return toast('emptyRoom');if(confirm(U.deleteAsk+'\n'+plan.items.filter(o=>o.roomId===selected.id).length+' '+U.deleteRoomContents)){const rid=selected.id;commit(p=>removeRoom(p,rid),'roomDeleted');choose(null);}break;
    
case 'delete-opening':commit(p=>p.openings=p.openings.filter(o=>o.id!==id));break;
    case 'edit-opening':{const o=plan.openings.find(o=>o.id===id),form=$('#opening-form');if(o&&form){form.dataset.editId=id;for(const k of ['side','type','offset','w','h','sill'])form.elements[k].value=o[k];form.dataset.initial=JSON.stringify(Array.from(new FormData(form)));form.scrollIntoView({block:'nearest',behavior:'smooth'});}break;}
    case 'clear':if(confirm(U.clearAsk)){commit(p=>p.items=[]);choose(null);}break;
    case 'add-room-side':case 'add-next-room':case 'add-room-shortcut':case 'add-room':{if(plan.rooms.length>=20)return toast('limit');if(selected?.kind==='room'&&action!=='add-room-side'){openModal(U.addAdjacentRoom,'<p class="micro">'+U.adjacentHelp+'</p><div class="room-side-picker">'+['north','west','east','south'].map(side=>button('add-room-side',U['addTo'+side],'plus','quiet','data-id="'+side+'"')).join('')+'</div>');return;}const form=$('#room-form');if(form&&selected?.kind==='room'){if(!form.reportValidity())return;const current=selectedRoom(),fields=form.elements,candidate={...current,name:fields.namedItem('name').value.trim(),w:Number(fields.namedItem('w').value),d:Number(fields.namedItem('d').value),measured:fields.namedItem('measured').checked};if(fields.namedItem('x'))candidate.x=Number(fields.namedItem('x').value);if(fields.namedItem('z'))candidate.z=Number(fields.namedItem('z').value);if(!candidate.name||!validRoom(plan,candidate))return toast('invalidRoom');if(JSON.stringify(current)!==JSON.stringify(candidate))updateRoom(candidate);}const bo=bounds(plan.rooms),r={id:uid(),name:U.newRoomName+' '+(plan.rooms.length+1),x:round(bo.x+bo.w),z:bo.z,w:3,d:3,kind:'living',floor:'oak',wall:'cream',measured:false,textureScale:1,textureAngle:0};if(action==='add-room-side')Object.assign(r,adjacentRoom(selectedRoom(),id,r.w,r.d));if(!validRoom(plan,r))return toast('adjacentBlocked');$('#modal').close();commit(p=>p.rooms.push(r));scene.editRooms=true;scene.fit('top');view='top';choose({kind:'room',id:r.id});drawer=false;refresh();$('#inspector').scrollTop=0;break;}
...
app.addEventListener('submit',e=>{e.preventDefault();if(scene?.walk||showBefore)return toast('notReady');if(['room-form','item-form','opening-form','global-form'].includes(e.target.id)&&saveRoomDraft(e.target.id))toast('changesSaved');});

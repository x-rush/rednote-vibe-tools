...
export type AppError = { code?: 'storage-recovery' | 'no-match' | 'content' | 'transition' | 'generic'; message: string; recoverable: boolean; reasons?: string[] }
export type AppState = { page: PageState; guild: GuildDomainState; error?: AppError; lastAwardedXp: number; newlyUnlockedBadgeIds: string[] }
...
export function appReducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    ...
    case 'OFFER_CREATED': return { ...state, guild: action.state, page: 'questOffer', error: undefined }
    case 'QUEST_ACCEPTED': return action.state.activeQuest ? { ...state, guild: action.state, page: 'questAccepted', error: undefined } : invalidTransition(state, '任务尚未成功接取。')
    case 'QUEST_SWAPPED': return { ...state, guild: action.state, page: 'questOffer', error: undefined }
    case 'QUEST_COMPLETED': return { ...state, guild: action.result.state, page: 'questComplete', lastAwardedXp: action.result.awardedXp, newlyUnlockedBadgeIds: action.result.newlyUnlockedBadgeIds, error: undefined }
    case 'QUEST_ABANDONED': return { ...state, guild: action.state, page: 'questAbandoned', error: undefined }
    case 'NAVIGATE': return { ...state, page: action.page, error: undefined }
    case 'RESTORE': {
      const guild = guildFromPayload(action.payload)
      return { ...state, guild, page: restorePage(action.payload), error: undefined }
    }
    case 'NO_MATCH': return { ...state, page: 'error', error: { code: 'no-match', message: '暂时没有完全合适的任务。', recoverable: true, reasons: action.reasons } }
    case 'FAIL': return { ...state, page: 'error', error: { code: 'generic', message: action.message, recoverable: action.recoverable } }

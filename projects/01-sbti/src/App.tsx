...
function ValidatedApp() {
  ...
  if (app.state.screen === 'result' && app.state.result) {
    ...
    return <ResultPage result={app.state.result} profile={profile} neighborLabel={neighborLabel} share={generateShareCardViewModel(app.state.result, content)} guide={copy.guide} identity={copy.identity} dimensionDefinitions={content.content.dimensions} onHome={() => app.dispatch({ type: 'HOME' })} onRestart={app.restart} />
  }

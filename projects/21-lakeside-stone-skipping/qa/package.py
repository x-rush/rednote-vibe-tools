...
files=sorted(p for p in dist.rglob('*') if p.is_file());assert (dist/'index.html').exists()
for p in files:assert p.suffix.lower() in allowed,p
assert 'qa-fixture' not in (dist/'index.html').read_text(encoding='utf-8')
archive=out/'lakeside-stone-skipping.zip'
...
manifest={'bytes':archive.stat().st_size,'sha256':hashlib.sha256(archive.read_bytes()).hexdigest(),'files':[p.relative_to(dist).as_posix() for p in files]}

...
class BudgetScene extends BaseScene{
  ...
  hit(e){const resize=this.resizeHit(e);if(resize)return resize;this.point(e);const hits=this.ray.intersectObjects(this.root.children,true);for(const h of hits){if(!h.object.visible||h.object.parent?.visible===false)continue;let data=h.object.userData;if(data.kind==='batch'){data=data.ranges.find(r=>h.faceIndex>=r.first&&h.faceIndex<r.last)?.data;if(!data?.kind)continue;return{...h,object:{userData:data}};}if(data.kind)return h;}return undefined;}
...
class FlatScene{
  ...
  previewOpening(o){this.openingPreview=o;this.draw();}
  hit(e){const p=this.point(e);for(const o of this.plan.openings){if(o.type!=='door')continue;const r=this.plan.rooms.find(r=>r.id===o.roomId);if(!r)continue;const horizontal=o.side==='north'||o.side==='south',line=horizontal?r.z+(o.side==='south'?r.d:0):r.x+(o.side==='east'?r.w:0),along=horizontal?p.x-r.x:p.z-r.z,across=horizontal?Math.abs(p.z-line):Math.abs(p.x-line);if(across<=Math.max(.06,5/this.scale)&&Math.abs(along-o.offset)<=o.w/2)return{object:{userData:{kind:'opening',id:o.id}},point:p};}return null;}
  draw(){if(!this.plan||document.hidden)return;const rect=viewportRect(this.host),w=rect.width,h=rect.height,dpr=Math.min(devicePixelRatio||1,1.5);this.canvas.width=Math.max(1,w*dpr);this.canvas.height=Math.max(1,h*dpr);this.canvas.style.width=w+'px';this.canvas.style.height=h+'px';const ctx=this.ctx;ctx.setTransform(dpr,0,0,dpr,0,0);ctx.fillStyle=this.night?'#c8cfbd':'#eaece1';ctx.fillRect(0,0,w,h);const b=bounds(this.plan.rooms);this.scale=Math.min((w-48)/b.w,(h-260)/b.d);this.scale=Math.max(8,this.scale);this.offsetX=(w-b.w*this.scale)/2-b.x*this.scale;this.offsetZ=(h-b.d*this.scale)/2-b.z*this.scale;
    ...
    for(const original of this.plan.openings){if(original.type!=='door')continue;const o=this.openingPreview?.id===original.id?this.openingPreview:original,r=this.plan.rooms.find(r=>r.id===o.roomId);if(!r)continue;const horizontal=o.side==='north'||o.side==='south',line=horizontal?r.z+(o.side==='south'?r.d:0):r.x+(o.side==='east'?r.w:0),center=(horizontal?r.x:r.z)+o.offset;ctx.strokeStyle=this.selected?.openingId===o.id?'#a76345':'#46583d';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(x(horizontal?center-o.w/2:line),z(horizontal?line:center-o.w/2));ctx.lineTo(x(horizontal?center+o.w/2:line),z(horizontal?line:center+o.w/2));ctx.stroke();}
    ctx.fillStyle='#69765e';ctx.font='11px sans-serif';ctx.fillText(this.catalog.ui.flatMode,18,h-185);
  }
  down(e){if(this.readonly||this.hit(e))return;const p=this.point(e);if(this.pending){this.cb.place(this.pending,p);return;}const r=roomAt(this.plan,p.x,p.z);let o=this.plan.items.slice().reverse().find(i=>Math.abs(i.x-p.x)<i.w/2&&Math.abs(i.z-p.z)<i.d/2);if(this.editRooms&&r&&this.selected?.id!==r.id){this.cb.select({kind:'room',id:r.id});return;}if(!this.editRooms&&o&&this.selected?.id!==o.id){this.cb.select({kind:'item',id:o.id});return;}if(this.editRooms&&r){this.cb.select({kind:'room',id:r.id});this.drag={kind:Math.abs(p.x-r.x-r.w)<.35&&Math.abs(p.z-r.z-r.d)<.35?'resize':'room',id:r.id,original:{...r},candidate:{...r},start:p};}else if(o){this.cb.select({kind:'item',id:o.id});this.drag={kind:'item',id:o.id,original:{...o},candidate:{...o},start:p};}else this.cb.select(r?{kind:'room',id:r.id}:null);this.canvas.setPointerCapture(e.pointerId);}

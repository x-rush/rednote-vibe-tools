...
- 定稿来源：用户确认的极简复古档案纸签图标 `public/assets/wuhualu/brand/app-icon-v2.png`，1024×1024 RGBA PNG。
- 核心构成：圆角墨黑底、米白档案纸签、单一金色圆标与右下朱印；无文字、伪铭文、器物拼贴或多余装饰。
- 正式导出：源图仅以 Lanczos 算法确定性缩小为 `release-assets/tool-icon-v1.png`（512×512）；128px 与 64px 缩略检查图仅用于 QA，不做二次生成或重绘。
- 二次检查：三档图标构图与源图一致；纸签外轮廓、金色圆标和朱印在 64px 下仍可分辨，透明圆角与安全边距完整。

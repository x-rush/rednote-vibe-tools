...
const maximumPrice = (base: number) => Math.floor(base * 1.4)
...
const conservativeDecision = (state: GameState): DailyDecision => {
  ...
  return {
    ...
    closeEarly: state.energy < 25,
    strategyId: 'cash-buffer',
  }
}

const advantageousDecision = (state: GameState): DailyDecision => ({
  menu: content.drinks.filter((product) => state.unlockedProductIds.includes(product.productId)).sort((left, right) => left.complexity - right.complexity || left.unitCost - right.unitCost).slice(0, 3).map((product) => ({ productId: product.productId, prepare: 4, price: maximumPrice(product.basePrice) })),
  closeEarly: state.energy < 12,
...
describe('three complete deterministic simulations', () => {
  it('covers an early bankruptcy route', () => {
    ...
    expect(run.results.length).toBeLessThan(100)
  ...
  it('covers a stronger route and reproduces its terminal snapshot', () => {
    ...
    expect(first.state.money).toBeGreaterThan(350)
    expect(first.state.reputation).toBeGreaterThanOrEqual(50)
    expect(first.state.currentEndingId).not.toBe('ending-hundred-days')

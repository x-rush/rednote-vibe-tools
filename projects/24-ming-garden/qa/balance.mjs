...
function spot(id){for(const rotated of [false,true])for(let pos=0;pos<e.slots(s);pos++)if(e.validPlace(s,id,pos,rotated))return {pos,rotated};return null;}
let minute=0;
...
await writeFile(new URL('./balance-results.json',import.meta.url),JSON.stringify(report,null,2));console.log(JSON.stringify({fullCollectionMinutes:minute,firstPerTier:report.firstPerTier,snapshots},null,2));

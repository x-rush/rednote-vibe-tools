...
export function LexiconScrollArtifact({ visual, points, onObserve }: { visual: LexiconScrollVisual; points: EvidenceObservationPoint[]; onObserve: (id: string) => void }) {
  ...
  return <div className="lexicon-scroll-artifact">
    ...
    {visual.entries.map((entry) => entry.id === activeId && <article key={entry.id} className="artifact-reading lexicon-entry"><span>材料记录</span><p>{entry.originalText}</p><mark>{entry.highlight}</mark><span>产品释文</span><p>{entry.interpretation}</p></article>)}
  </div>
}
